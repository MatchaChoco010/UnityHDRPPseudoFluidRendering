...
namespace UnityEngine.Experimental.Rendering.HDPipeline {
    ...
    public partial class HDRenderPipeline : UnityEngine.Rendering.RenderPipeline {
        ...
        ShaderTagId[] m_AllTransparentPassNames = {
        ...
        ShaderTagId[] m_TransparentNoBackfaceNames = {
            HDShaderPassNames.s_ForwardOnlyName,
            HDShaderPassNames.s_ForwardName,
            HDShaderPassNames.s_SRPDefaultUnlitName
        };

        ShaderTagId[] m_AllForwardOpaquePassNames = {
            HDShaderPassNames.s_ForwardOnlyName,
            HDShaderPassNames.s_ForwardName,
            HDShaderPassNames.s_SRPDefaultUnlitName
        };
        ...
        void UpgradeResourcesIfNeeded () {
            ...
            if ((GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineResources == null)
                (GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineResources = UnityEditor.AssetDatabase.LoadAssetAtPath<RenderPipelineResources> (HDUtils.GetHDRenderPipelinePath () + "Runtime/RenderPipelineResources/HDRenderPipelineResources.asset");
            ...
            if ((GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineRayTracingResources == null)
                (GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineRayTracingResources = UnityEditor.AssetDatabase.LoadAssetAtPath<HDRenderPipelineRayTracingResources> (HDUtils.GetHDRenderPipelinePath () + "Runtime/RenderPipelineResources/HDRenderPipelineRayTracingResources.asset");
            ...
            if ((GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineEditorResources == null)
                (GraphicsSettings.renderPipelineAsset as HDRenderPipelineAsset).renderPipelineEditorResources = UnityEditor.AssetDatabase.LoadAssetAtPath<HDRenderPipelineEditorResources> (HDUtils.GetHDRenderPipelinePath () + "Editor/RenderPipelineResources/HDRenderPipelineEditorResources.asset");
        ...
        void CopyDepthBuffer (CommandBuffer cmd) {
            using (new ProfilingSample (cmd, "Copy depth buffer", CustomSamplerId.CopyDepthBuffer.GetSampler ())) {
                // TODO: maybe we don't actually need the top MIP level?
                // That way we could avoid making the copy, and build the MIP hierarchy directly.
                // The downside is that our SSR tracing accuracy would decrease a little bit.
            ...
            using (ListPool<CameraPositionSettings>.Get (out List<CameraPositionSettings> cameraPositionSettings)) {
                ...
                void AddHDProbeRenderRequests (
                    ...
                    for (int j = 0; j < cameraSettings.Count; ++j) {
                        ...
                        camera.GetComponent<HDAdditionalCameraData> ().flipYMode = visibleProbe.type == ProbeSettings.ProbeType.ReflectionProbe ?
            ...
            using (ListPool<RTHandle>.Get (out var aovBuffers)) {
                ...
                if (m_CurrentDebugDisplaySettings.IsDebugMaterialDisplayEnabled () || m_CurrentDebugDisplaySettings.IsMaterialValidationEnabled ()) {
                    ...
                    if (hdCamera.frameSettings.BuildLightListRunsAsync ()) {
                        ...
                        void Callback (CommandBuffer asyncCmd) => BuildGPULightListsCommon (hdCamera, asyncCmd, m_SharedRTManager.GetDepthStencilBuffer (hdCamera.frameSettings.IsEnabled (FrameSettingsField.MSAA)), m_SharedRTManager.GetStencilBufferCopy ());
                    }
                    ...
                    if (hdCamera.frameSettings.VolumeVoxelizationRunsAsync ()) {
                        ...
                        void Callback (CommandBuffer asyncCmd) => VolumeVoxelizationPass (hdCamera, asyncCmd, m_FrameCount, densityVolumes);
                    }
                    ...
                    if (hdCamera.frameSettings.SSRRunsAsync ()) {
                        ...
                        void Callback (CommandBuffer asyncCmd) => RenderSSR (hdCamera, asyncCmd, renderContext);
                    }
        ...
        static void RenderDepthPrepass (ScriptableRenderContext renderContext,
            ...
            RTHandle depthBuffer, in RendererList depthOnlyRendererList, in RendererList mrtRendererList,
            bool hasDepthOnlyPass
#if ENABLE_RAYTRACING
            , HDRaytracingManager rayTracingManager, in RendererList rayTracingOpaqueRL, in RendererList rayTracingTransparentRL
#endif
        ) {
        ...
        void RenderMyPass (CullingResults cull, HDCamera hdCamera, ScriptableRenderContext renderContext, CommandBuffer cmd) {
            ...
            using (new ProfilingSample (cmd, m_CurrentDebugDisplaySettings.IsDebugDisplayEnabled () ? "MyPass Debug" : "MyPass", CustomSamplerId.GBuffer.GetSampler ())) {
                ...
                var depthStencilBuffer = m_SharedRTManager.GetDepthStencilBuffer ();

                cmd.GetTemporaryRT (
                    Shader.PropertyToID ("_Depth0RT"),
                    width : depthStencilBuffer.rt.width,
                    height : depthStencilBuffer.rt.height,
                    depthBuffer : 0,
                    filter : FilterMode.Point,
                    format : GraphicsFormat.R32_SFloat
                );
                cmd.GetTemporaryRT (
                    Shader.PropertyToID ("_Depth1RT"),
                    width : depthStencilBuffer.rt.width,
                    height : depthStencilBuffer.rt.height,
                    depthBuffer : 0,
                    filter : FilterMode.Point,
                ...
                // Depth and Stencil
                HDUtils.SetRenderTarget (
                    cmd,
                    new [] {
                        new RenderTargetIdentifier (Shader.PropertyToID ("_Depth0RT"))
                    },
                    depthStencilBuffer,
                    ClearFlag.Color,
                    new Color (1, 1, 1, 1)
                );
                var rendererList = RendererList.Create (CreateOpaqueRendererListDesc (cull, hdCamera.camera, HDShaderPassNames.s_MyDepthPassName, m_CurrentRendererConfigurationBakedLighting));
                DrawOpaqueRendererList (renderContext, cmd, hdCamera.frameSettings, rendererList);
                ...
                HDUtils.DrawFullScreen (
                    ...
                    m_MyMaterial,
                    new [] {
                        new RenderTargetIdentifier (Shader.PropertyToID ("_Depth1RT"))
                    },
                    depthStencilBuffer,
                    shaderPassId : 4
                );
                ...
                cmd.SetGlobalVector (
                    Shader.PropertyToID ("_FrustumCorner"),
                    corner
                );
                HDUtils.DrawFullScreen (
                    cmd,
                    m_MyMaterial,
                    m_GbufferManager.GetBuffersRTI (hdCamera.frameSettings),
                    depthStencilBuffer,
                    shaderPassId : 6
                );
